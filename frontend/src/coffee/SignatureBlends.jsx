    ...
    <div className="speciality-page">
      ...
      <section className="sp-collection">
        <div className="sp-collection-header">
          <h2 className="sp-section-title">Signature Blends</h2>
          <div className="sp-divider"></div>
        </div>

        <div className="sp-grid">
          {collection.map((item) => (
            /* Wrapped the article in a Link to go to Signature Blends Detail */
            <Link to={`/signatureblendsdetail/${item.id}`} key={item.id} style={{textDecoration: 'none'}}>
